...
- **Open issue**: old boot files remain alongside new ones. GRUB lists 4 kernels instead of 2. Leftover files: `vmlinuz-6.12.103_1`, `vmlinuz-6.18.44_1`, `initramfs-6.12.103_1.img`, `initramfs-6.18.44_1.img`, `config-6.12.103_1`, `config-6.18.44_1` (~136MB).
- Void's kernel packages replace vmlinuz in-place but initramfs/config are generated by post-install hooks that don't clean up old versions. Other distros (Arch, Debian) handle this automatically.
- No built-in Void mechanism for cleanup. Need a post-update hook or script to prevent accumulation.
- User will reboot first to confirm everything boots, then revisit cleanup.
- Also: WireGuard/Mullvad connection was temporarily slow, resolved itself (transient). Vault docs at `codex/linux/void/manuals/void-wireguard.md` have no performance tuning section. Also: Mullvad speedtest URL `am.i.mullvad.net/speedtest` returns 404.
